...
{    
  ...
  static int Voltage_Temp,Voltage_Count,Voltage_All; 	  //电压测量相关变量
	...
	{   
		...
		Encoder_Right=-Read_Encoder(4);                     //读取编码器的值 //左轮A相接TIM2_CH1,右轮A相接TIM4_CH2,故这里两个编码器的极性相同
		Get_Velocity_Form_Encoder(Encoder_Left,Encoder_Right);//编码器读数转速度（mm/s）
		if(delay_flag==1)
		{
			if(++delay_50==10)	 delay_50=0,delay_flag=0;     //给主函数提供50ms的精准延时，示波器需要50ms精确延时
		}
		if(Flag_Target==1)                                     
		{
			...
			return 0;	                                               
																											  ...
																											  //Encoder_Left/Encoder_Right极性：前进为正，后退为负
		Read_Distane();                                     //获取超声波测量距离值
		if(Flag_avoid==0&&Flag_follow==0)	Led_Flash(100);   //LED闪烁;常规模式 1s改变一次指示灯的状态	
		if(Flag_avoid==1||Flag_follow==1)	Led_Flash(0);     //LED常亮;避障模式	
...
返回  值：速度控制PWM
**************************************************************************/
//修改前进后退速度，请修Target_Velocity，比如，改成60就比较慢了
int Velocity(int encoder_left,int encoder_right)
{  
	 static float velocity,Encoder_Least,Encoder_bias,Movement;
	 static float Encoder_Integral,Target_Velocity;
	//==================遥控前进后退部分==================// 
   if(Flag_follow==1||Flag_avoid==1)  Target_Velocity = 30;      		//如果进入跟随模式,自动降低速度
	...
	//=============速度PI控制器=======================//	
	 ...
	 Encoder_bias += Encoder_Least*0.14;	                                  //一阶低通滤波器  
